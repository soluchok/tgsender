		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			return client.Run(cmd.Context(), func(ctx context.Context) error {
				...
				for scanner.Scan() {
					...
					for len(phones) > 15 {
						if err := ImportContactsAndSave(ctx, client.API(), out, retry, phones[:15], savedPhoneSet); err != nil {
							return err
						}
				...
				if err := ImportContactsAndSave(ctx, client.API(), out, retry, phones, savedPhoneSet); err != nil {
					return err
				}
...
func ImportContactsAndSave(ctx context.Context, api *tg.Client, out, retry io.Writer, phones []string, phoneSet map[string]struct{}) error {
	res, err := ImportContacts(ctx, api, phones)
	if err != nil {
		return fmt.Errorf("failed to import contacts: %w", err)
	}

	for _, id := range res.GetRetryContacts() {
		retry.Write([]byte(phones[id]))
		retry.Write([]byte{'\n'})
	}

	for _, user := range slices.Convert(res.GetUsers(), toUser) {
